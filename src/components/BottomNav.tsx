...
export default function BottomNav({ nightMode, activeTab, currentDay }: BottomNavProps) {
  ...
  return (
    <div className="fixed bottom-4 left-0 right-0 z-50 flex justify-center px-4">
      ...
      >
        <Link href="/dashboard" className={`flex flex-col items-center gap-0.5 py-2 px-3 rounded-2xl transition-colors ${activeTab === 'home' ? activeClass : inactiveClass}`}>
          <Home className="w-5 h-5" strokeWidth={activeTab === 'home' ? 2.5 : 1.5} />
          <span className="text-[9px] font-medium">Home</span>
        </Link>

        <Link href={`/day/${currentDay}`} className={`flex flex-col items-center gap-0.5 py-2 px-3 rounded-2xl transition-colors ${activeTab === 'today' ? activeClass : inactiveClass}`}>
          <Target className="w-5 h-5" strokeWidth={activeTab === 'today' ? 2.5 : 1.5} />
          <span className="text-[9px] font-medium">Today</span>
        </Link>

        <Link href={`/day/${currentDay}`} className="relative -top-5 flex flex-col items-center">
        ...
        <Link href="/profile" className={`flex flex-col items-center gap-0.5 py-2 px-3 rounded-2xl transition-colors ${activeTab === 'profile' ? activeClass : inactiveClass}`}>
